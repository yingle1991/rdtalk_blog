			...
			<view class="other-wrapper">
				...
				<view class="list row">
					...
					<view class="item column center" @click="mpWxGetUserInfo">
						<image class="icon" src="/static/icon/wechat.png"></image>
					</view>
					<!-- #endif -->
...
<script>
	...
	export default{
		...
		methods: {
			...
			mobileLogin() {
				if (!this.agreement) {
					this.$mHelper.toast('请阅读并同意用户服务及隐私协议');
					return;
				}
				this.$refs.form.validate().then(() => {
					...
					const loginPromise = this.loginType == 'password' ? login(mobile, password,this.mpWxCode) :
			...
			loginSuccessCallBack(data) {
				this.$util.msg('登录成功');
				this.$mStore.commit('setToken', data);
